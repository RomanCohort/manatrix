...
在50个测试场景上的实验评估表明，完整配置的Manatrix系统达到95%响应质量，而纯LLM配置为100%但缺乏专业化。消融研究表明，专家路由系统显著提升回答质量，B4（20专家系统）平均质量达95%，路由准确率50%。在8个安全场景的真实DeepSeek API测试中，专家系统能够正确识别并路由到对应领域专家，如SQL注入→Web安全、横向移动→AD安全、凭据攻击→密码破解等。
...
**专家系统（真实DeepSeek API测试）**

基于DeepSeek API的真实实验（2026-06-05，8个安全场景，64次实验）：

| 配置 | 平均响应质量 | 专家路由准确率 | 平均响应时间 | Token消耗 |
|------|-------------|---------------|-------------|----------|
| B1: 单LLM（无专家） | 99.6% | 0% | 13.8s | 21,146 |
| B2: 单专家（侦察） | 72.0% | 12.5% | 17.5s | 25,293 |
| B3: 三专家系统 | 66.4% | 62.5% | 11.4s | 16,714 |
| B4: 20专家MoE系统 | **95.2%** | 50.0% | 15.8s | 23,491 |

**专家路由实例**：
- SQL注入检测 → `web_application` 专家（置信度95%）
- 权限提升 → `exploitation` 专家（置信度90%）
- 横向移动 → `lateral_movement` / `active_directory` 专家
- 凭据攻击 → `credential` 专家（置信度95%）
- 网络侦察 → `reconnaissance` 专家（置信度70%）
- EDR绕过 → `exploitation` 专家（置信度85%）
- API安全 → `api_security` 专家（置信度95%）

**关键发现**：
1. B4（20专家系统）响应质量最高（95.2%），证明专业化有效
2. B3路由准确率最高（62.5%），简单规则更稳定
3. B1质量高但缺乏领域专业化
4. 专家系统平均响应时间15-17秒，适合渗透测试场景
