"""
Manatrix Jupyter Kernel

Provides a Jupyter kernel for interactive Manatrix sessions.
"""
...
class ManatrixKernel(Kernel):
    """Jupyter kernel for Manatrix CLI."""
...
Commands:
  !pg <command>  - Run manatrix CLI command
        ...
        if code.strip().startswith("!"):
            ...
            try:
                result = subprocess.run(
                    [sys.executable, "-m", "manatrix.cli"] + parts,
            ...
            if match:
                ...
                result = subprocess.run(
                    [sys.executable, "-m", "manatrix.cli", "demo", topic],
                    capture_output=True, text=True, timeout=30
                )
...
if __name__ == "__main__":
    IPKernelApp.launch_instance(kernel_class=ManatrixKernel)